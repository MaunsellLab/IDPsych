...
% the name R0 is kind of confusing - Lai
...
plot(resp, 'o-');
hold on;
plot([0, steps], [0.2222, 0.2222], 'r:');
plot([0, steps], [zeroline, zeroline], 'k:');
set(gca, 'xTick', 0:1:steps);
set(gca, 'XTickLabel', xTickLabels);
xlabel('Dots Direction (deg)');
set(gca, 'yTick', [0, min(zeroline, 0.2222), max(zeroline, 0.2222), 1], 'yTickLabel', ...
{'0', sprintf('%.2f', min(zeroline, 0.2222)), sprintf('%.2f', max(zeroline, 0.2222)), 'Rmax'});
